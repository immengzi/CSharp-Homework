...
namespace OrderManage
{
    class Program
    {
            ...
            while (true)
            {
                ...
                try
                {
                    switch (cho)
                    {
                        ...
                        case "6": Show(orderService); break;
                        case "7": Export(orderService); break;
                        case "8": Import(orderService); break;
                    }
                }
                catch (Exception e)
                {
        ...
        static void Export(OrderService orderService)
        {
            orderService.Export();
        }
        static void Import(OrderService orderService)
        {
            orderService.Import();
        }
        static IEnumerable<Order> Searchorder(OrderService orderService)
        {
            Console.WriteLine("1.根据订单号查找\n2.根据商品查找\n3.根据用户查找");
            string cho = Console.ReadLine();
            IEnumerable<Order> p;
            switch (cho)
            {
